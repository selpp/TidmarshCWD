...
def wardenNetworkCallback(nature, data, conn = None):
    ...
    if(nature == network.PACKET_TYPE_CONNECTION_CLOSE):
        debug("[WARDEN] Closing connection "+str(conn))
        conn.close()
        return
    ...
    if(nature == network.PACKET_TYPE_PLUG_REQUEST): #request to plug a wp to another
        ...
        if(remoteWPW == warden_name):
            debug("[WARDEN] NOTE: "+str(warden_name)+" is the local warden, using self")
            remoteWPW = "self"
        
        debug("[WARDEN] Plug request: "+str(localWP)+" --> "+str(remoteWP)+"@"+str(remoteWPW), level = 2)
...
def setTags():
    global wtags 
    debug("[WARDEN] Detecting tags...", 0)
    try:
        
        if(len(get_available_gpus()) > 0):
            wtags.append("gpu")
            debug("[WARDEN] This warden has GPU capability", 0)
    except:
        pass            
